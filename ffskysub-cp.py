...
# add artificial Lyman alpha emission

artifu, artamp = "027", "RL"
#tmparray = sky_spectra[(ifuslots==artifu)&(amps==artamp)][0][55].copy()


def gauss(A, mid, sig, def_wave):
	return interp1d(def_wave, A/((2*np.pi)**(1/2)*sig)* np.exp(-1/2*((def_wave-mid)/sig)**2), fill_value = "extrapolate")

gaussamp, gaussmid, gausssigma = 150, 4800, 8
sky_spectra_orig[(table["ifuslot"]==artifu)&(table["amp"]==artamp)] += gauss(gaussamp, gaussmid, gausssigma, def_wave)(wavelength_orig[(table["ifuslot"]==artifu)&(table["amp"]==artamp)])

#plt.figure()
#plt.plot(wavelength[(ifuslots==artifu)&(amps==artamp)][0][55], sky_spectra[(ifuslots==artifu)&(amps==artamp)][0][55] - tmparray)
#plt.plot(wavelength[(ifuslots==artifu)&(amps==artamp)][0][55], gauss(50, 4500, 4, def_wave)(wavelength[(ifuslots==artifu)&(amps==artamp)][0][55]))
#plt.xlim(4450, 4550)
#plt.show()
...
for i in range(len(sky_spectra)): #order:#range(len(sky_spectra))[START:STOP]:
	new_skysub = (sky_spectra[i] - new_sky_iter[i]*(1+rescor[(ifuslots[i], amps[i])]))
	if FILTER:
	...
	if INTERPOLATE:
		...
		new_skysub_int.append(rebinned_skysub)
		#new_skysub_int_rel.append(get_rebinned(new_skysub_rel[i]))
		f2f_int.append(get_rebinned(i,fiber_to_fiber[i])[1])
		nsi_int.append(get_rebinned(i,new_sky_iter[i])[1])
...
plt.figure()
plt.plot(def_wave, np.nanmean(second_skysub_iter[(ifuslots==artifu)&(amps==artamp)][0], axis=0))
plt.plot(def_wave, gauss(gaussamp, gaussmid, gausssigma, def_wave)(def_wave)/2., color="black")
plt.show()
...
if SAVEASPICKLE:
	for j,i in enumerate(order):
		...
		filename = open('../skysubtractedfiles/{}/{}/xsubr_multi_xxx_{}_xxx_{}_rebin.pickle'.format(shot, exp, thisifu, thisamp),'wb')
		pickle.dump((ww, {'sky_subtracted':thissubtracted,'sky_spectrum':new_sky_iter_int[i],'fiber_to_fiber':fiber_to_fiber_int[i]}), filename)
		print('wrote to ../skysubtractedfiles/{}/{}/xsubr_multi_xxx_{}_xxx_{}_rebin.pickle'.format(shot, exp, thisifu, thisamp))
