        ...
        for amp in aa: #['LL']:
            ...
            try:
                ...
                for i in range(len(gg)):
                    fin = "../xsky/{}v{}/{}/x_{}".format(gg[i].split("/")[-6],gg.split("/")[-4][-3:],gg.split("/")[-3],gg.split("/")[-1])
                    #fin = gg[i]  # this is specfic for the 20180822 shots... CHANGE IT!!!
                    hdu = fits.open(fin)
                    hdu.append(fits.ImageHDU(allrescaled[i], name='pca_sky_spectrum'))
                    #hdu.append(fits.ImageHDU(allrepoly[i], name='pcarepoly'))
                    hdu.writeto('x_{}'.format(fin.split('/')[-1]), overwrite=True)
